...
class Simulation_rl:
    ...
    def calculate_efficiency_reward(self,previous_state: State, current_state: State) -> float:
       efficient_reward_function_version=int(os.getenv("EFFICIENCY_REWARD_FUNCTION_VERSION",3))
       if efficient_reward_function_version==1:
            return self.calculate_efficiency_reward_v1(previous_state,current_state)
       elif efficient_reward_function_version==2:
            return self.calculate_efficiency_reward_v2(previous_state,current_state)
       else:
            return self.calculate_efficiency_reward_v3(previous_state,current_state)

    def calculate_efficiency_reward_v1(self, previous_state: State, current_state: State) -> float:
        delivery_destination = self.target_delivery[1].delivery_destination
        current_distance = (manhattan_distance(self.delivery_vehicle.x, self.delivery_vehicle.y, delivery_destination.x,
                                               delivery_destination.y))
        max_manhattan_distance = (
            manhattan_distance(self.agent_checkpoint_position[0], self.agent_checkpoint_position[1],
                               delivery_destination.x, delivery_destination.y))
    ...
    def calculate_efficiency_reward_v2(self, previous_state: State, current_state: State) -> float:
        ...
        previous_distance = manhattan_distance(delivery_vehicle_position_at_previous_state[0],
                                               delivery_vehicle_position_at_previous_state[1], delivery_destination.x,
                                               delivery_destination.y)
        current_distance = (manhattan_distance(self.delivery_vehicle.x, self.delivery_vehicle.y, delivery_destination.x,
                                               delivery_destination.y))
        max_manhattan_distance = (
