...
class State:
    ...
    def scale_state_values(self):
        ...
        #use this when reverse is enabled
        scaled_delivery_vehicle_velocity= self.__delivery_vehicle_velocity/self.__delivery_vehicle_max_vel
        ...
        return np.array([scaled_distance_to_target_delivery,scaled_sensor_one_data,scaled_sensor_two_data,scaled_sensor_three_data,
                ...
                scaled_delivery_vehicle_velocity,scaled_delivery_vehicle_steering_angle,
                         scaled_delivery_vehicle_location_x
                ,scaled_delivery_vehicle_location_y,scaled_target_delivery_location_x,scaled_target_delivery_location_y,
                         scaled_current_location_speed_limit],dtype=np.float32)
    ...
    def __scale_sensor_data(self,sensor_data):
        ...
        if sensor_distance_to_obstacle<=Constant.SENSORS_COLLISION_DISTANCE:
            return 0
         #returns 1 when sensor is far from obstacle
        return 1-(Constant.MAX_SENSOR_DISTANCE - sensor_distance_to_obstacle) / (Constant.MAX_SENSOR_DISTANCE - Constant.SENSORS_COLLISION_DISTANCE)
    ...
    def get_sensor_six_data(self):
        return self.__sensor_six_data

    def get_sensor_seven_data(self):
        return self.__sensor_seven_data

    def get_sensor_eight_data(self):
        return self.__sensor_eight_data
